...
exports.verifyTwoStepOTP = async (req, res) => {
  try {
    ...
    return res.status(200)
      .cookie("accessToken", accessToken, { httpOnly: true, secure: true, maxAge: 0.5 * 60 * 1000, sameSite: "Strict" })
      .cookie("refreshToken", refreshToken, { httpOnly: true, secure: true, maxAge: 15 * 24 * 60 * 60 * 1000, sameSite: "Strict" })
      .json({ success: true, status: 200, message: "2-step verification successful. Login completed.", user: loginResult.userData, token: accessToken });
...
exports.userLogin = async (req, res) => {
  try {
    ...
    if (checkEmailIsExist.twoStepEnabled) {
      ...
      const encryptedOtpOptions = otpOptions.map(o => Buffer.from(o.toString()).toString('base64'));

      checkEmailIsExist.twoStepOtp = otp;
      checkEmailIsExist.twoStepOtpExpiry = new Date(Date.now() + 10 * 60 * 1000); // 10 minutes expiry
      await checkEmailIsExist.save();
      ...
      return res.status(200).json({
        status: 200,
        success: true,
        message: "2-step verification required. Please enter the OTP sent to your email or Sms.",
  ...
  try {
    ...
    if (!checkUser) {
      checkUser = await user.create({
        ...
        gender,
        phoneNo,
        plan: "6874b3a50ba9a080d12076cc", // Assign default plan on creation
        startDate: null,
        endDate: null,
