...
const Header = () => {
  ...
  return (
    <header className="bg-primary-dark/10 backdrop-blur-lg  text-white shadow-lg shadow-black/30 fixed w-full z-50">
      <div className="container mx-auto flex justify-between items-center py-4 px-4 md:px-6">
          ...
          >
            <div className="w-8 h-8 rounded-full flex items-center justify-center">
              {currentUser?.photo && currentUser?.photo !== "null" ? (
                ...
                <span className="text-base w-8 h-8 font-bold uppercase group-hover:border-[#8A775A] border-2 rounded-full flex justify-center items-center">
            ...
            {currentUser && (
              <span className="hidden md600:block font-medium capitalize transition-colors cursor-pointer">
                {decryptData(currentUser?.userName)}
              </span>
            )}
            <span className="absolute left-0 -bottom-1 w-0 h-[2px] bg-[#8A775A] transition-all duration-300 group-hover:w-full" />
              ...
              <div className="bg-[#1e1e1e] rounded-[2px] p-[16px] sm:p-[24px] w-[90%] max-w-[400px] text-white shadow-lg">
                ...
                <div className="flex justify-between gap-4 mt-4">
                  ...
                  <button
                    ...
                    className="w-full text-white py-2 rounded-[4px] text-[14px] font-medium sm:py-3 bg-white/30 hover:bg-white/40 border-none cursor-pointer transition-all duration-400 ease-in-out"
                  >
                    Yes, Logout
                  </button>
                </div>
        ...
        <Modal
          ...
          className="fixed inset-0 bg-black/10  flex items-center justify-center z-50 loginModal"
        >
